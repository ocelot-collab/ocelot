...
kwargs={'xlamds':(h_eV_s * speed_of_light / E_pohoton), #[m] - central wavelength
        'rho':1.0e-4, 
        'shape':(551,551,1),             #(x,y,z) shape of field matrix (reversed) to dfl.fld
...
MirrorSurface = ImperfectMirrorSurface(hrms=2e-9, angle=10*np.pi/180, plane='x', eid='MirrorSurface')
Aperture = ApertureRect(lx=4000e-6, ly=2000e-6, eid='Aperture')
...
FreeSpace_to_the_sample = FreeSpace(l=l_b, mx=0.05, my=0.15, eid='free_space_to_the_sample')
...
plot_dfl(dfl_prop, fig_name='{}'.format(FreeSpace_before_lens.eid), phase=True)
