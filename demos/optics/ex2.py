...
def init_geometry():
    ...
    m1 = EllipticMirror(r=[0,0,0*cm], size=[2*cm,1*cm,30*mm], no=[0.0,1,0], a=[100*cm, 10.0*cm ], id="m1")
    m2 = EllipticMirror(r=[0,0,0*cm], size=[2*cm,50*cm,30*mm], no=[0.0,1,0], a=[5*m, 1.*cm ], id="m2")
    #m2 = EllipticMirror(r=[0,25*cm,2*m], size=[2*cm,110*cm,30*mm], no=[0.0,1,0], a=[100*cm, -10.0*cm ], id="m2")
    m3 = Mirror(r=[0,5*mm,3*m], size=[9*cm,9*cm,1*mm], no=[0.0,0.0,-1.0], id="m3")
...
for i in xrange(40):
    ...
    r = Ray(r0=[0,a[1],-sqrt(a[0]**2 - a[1]**2)], k=[0,-a[1]/a[0] * (1+ np.random.randn()*0.1) ,1])
    #test 1 -0.0359695
