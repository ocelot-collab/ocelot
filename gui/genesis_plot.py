...
def plot_gen_out_all(handle=None,savefig='png',showfig=False,choice=(1,1,1,1,6.05,1,0,0,0,0,0),vartype_dfl=complex128,debug=1):
    ...
    for handle in handles:
        ...
        if isinstance(handle,GenesisOutput):
            ...
            if choice[1]: f1=plot_gen_out_ph(handle,savefig=savefig,debug=debug)
        ...
        if os.path.isfile(handle.filePath+'.dfl') and any(choice[5:8]):
            ...
            if choice[5]: f5=plot_dfl(dfl,savefig=savefig,debug=debug)
    ...
    # plt.draw()
    if savefig!=False:
        if debug>0: print('    plots recorded to *.'+str(savefig)+' files')
            
    if showfig:
        if debug>0: print('    showing plots, close all to proceed')
        plt.show()
    # else:
        # plt.close('all')
...
def plot_gen_out_evo(g, params=['und_quad','el_size','el_energy','el_bunching','rad_pow_en','rad_spec','rad_size','rad_spec_evo_n','rad_pow_evo_n'], figsize=4, legend = False, fig_name = None, savefig=False, showfig=False,debug=1):
    ...
    if fig_name is None:
        if g.fileName() is '':
            ...
            if debug>0: print('    plotting '+params_str)
        else:
            fig = plt.figure(g.fileName()+'_'+params_str)
            if debug>0: print('    plotting '+g.fileName()+'_'+params_str)
    else:
        fig = plt.figure(fig_name)
        if debug>0: print('    plotting '+fig_name)
...
def plot_edist(edist, figsize=4, fig_name = None, savefig=False, showfig=False, scatter=False, plot_x_y=True, plot_xy_s=True, bins=(50,50,50,50), flip_t=True, beam_E_plot='eV', cmin=0, debug=1):
    ...
    if beam_E_plot=='eV':
        ...
        if scatter: ax_se.scatter(s, edist.g,marker='.')
        else: ax_se.hist2d(s, edist.g, [bins[2],bins[3]],cmin=cmin)
        ax_se.set_xlabel('s [$\mu$m]')
        ax_se.set_ylabel('$\gamma$')
    
    if plot_xy_s:
        ax_xs=fig.add_subplot(2, 1+plot_x_y+plot_xy_s, 4+plot_x_y,sharex=ax_curr)
        if scatter: ax_xs.scatter(s, 1e6*edist.x,marker='.')
        else: ax_xs.hist2d(s, 1e6*edist.x, [bins[2],bins[0]],cmin=cmin)
        ax_xs.set_xlabel('s [$\mu$m]')
        ax_xs.set_ylabel('x [$\mu$m]')
        
        ax_ys=fig.add_subplot(2, 1+plot_x_y+plot_xy_s, 2,sharex=ax_curr)
        if scatter: ax_ys.scatter(s, 1e6*edist.y,marker='.')
        else: ax_ys.hist2d(s, 1e6*edist.y, [bins[2],bins[1]],cmin=cmin)
        ax_ys.set_xlabel('s [$\mu$m]')
        ax_ys.set_ylabel('y [$\mu$m]')
        
    if plot_x_y:
        ax_xy=fig.add_subplot(2, 1+plot_x_y+plot_xy_s, 2+plot_xy_s)
        if scatter: ax_xy.scatter(edist.x*1e6, edist.y*1e6,marker='.')
        else: ax_xy.hist2d(edist.x*1e6, edist.y*1e6, [bins[0],bins[1]],cmin=cmin)
        ax_xy.set_xlabel('x [$\mu$m]')
        ax_xy.set_ylabel('y [$\mu$m]')
        
        ax_pxpy=fig.add_subplot(2, 1+plot_x_y+plot_xy_s, 4+2*plot_xy_s)
        if scatter: ax_pxpy.scatter(edist.xp*1e6, edist.yp*1e6,marker='.')
        else: ax_pxpy.hist2d(edist.xp*1e6, edist.yp*1e6, [bins[0],bins[1]],cmin=cmin)
    ...
    if savefig!=False:
        if savefig==True:
            savefig='png'
        if debug>1: print('      saving '+edist.fileName()+'.'+savefig)
