...
def init_plots(views, geo):
    ...
    for iview in xrange(nviews):
        ...
        scene.ax[iview] = scene.fig.add_subplot(view_id, autoscale_on=False)

        if views[iview] == 'geometry':
            scene.line_wf, = scene.ax[iview].plot([], [], '.', lw=2)
            plot_geometry(scene.ax[iview], geo)
            scene.ax[iview].grid()
            scene.time_template = 't = %.1f'
...
def plot_geometry(ax, geo, scales = [1,1,1]):
    ...
    for o in geo():
        ...
        zmax = np.max( map(lambda x: x.r[2] + x.size[1],geo))
        zmin = np.min( map(lambda x: x.r[2] - x.size[1],geo))
...
def plot_rays(ax, rays):
    ...
    for r in rays:
        for i in xrange(len(r.r0)):
            print 'plotting ray!', r.r0[i], r.k[i], r.s[i]
