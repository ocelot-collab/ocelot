...
class BendAtom(Magnet):
    ...
    def _curvature(self):
        return 0.0 if self.l == 0.0 else self.angle / self.l

    def _R_edge(self, fint, edge, xp=np):
        return bend_edge_matrix(self._curvature(), edge, gap=self.gap, fint=fint, xp=xp)

    def linear_r_entrance(self, energy: float = 0.0, xp=np):
        return self._R_edge(self.fint, self.e1, xp=xp)

    def linear_r_exit(self, energy: float = 0.0, xp=np):
        return self._R_edge(self.fintx, self.e2, xp=xp)
    ...
    def create_second_order_entrance_params(self, energy: float, delta_length: float = 0.0) -> SecondOrderParams:
        ...
        _, T = fringe_ent(h=self._curvature(), k1=self.k1, e=self.e1, h_pole=self.h_pole1,
    ...
    def create_second_order_exit_params(self, energy: float, delta_length: float = 0.0) -> SecondOrderParams:
        ...
        _, T = fringe_ext(h=self._curvature(), k1=self.k1, e=self.e2, h_pole=self.h_pole2,
