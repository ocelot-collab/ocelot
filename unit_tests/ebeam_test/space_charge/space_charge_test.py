...
def test_track_with_sp(lattice, p_array, parameter=None, update_ref_values=False):
    ...
    # The mean py crosses zero; FFT/reduction roundoff can exceed a purely
    # relative tolerance there while remaining below 1e-15 in absolute terms.
    result1 = check_dict(tws_track, tws_track_p_array_ref['tws_track'], 1e-6,
                         assert_info=' tws_track - ', absolute_tolerance=1e-15)
